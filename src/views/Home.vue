...
import HomeWorkplaceAnimation from "@/components/animations/OfficeWorkspace.vue";
import HomeAboutSection from "@/components/HomePage/AboutSection.vue";
import HomeSkillsSection from "@/components/HomePage/SkillsSection.vue";
import HomeContactSection from "@/components/HomePage/ContactSection.vue";
...
const homePageContent = reactive([]),
  homeAboutSectionContent = reactive([]),
  languageCode = ref("");
const filterArrayContent = (array, final) => {
  array
    .filter((content) => content.code == (languageCode.value || Language))
    .map((content) => final.push(content));
},
  getLanguage = () => {
              ...
              <h1 class="text-5xl font-semibold my-4">
                ...
                <span :class="[
                  props.themeColor == 'light'
                    ? 'text-blue-600'
                    : 'text-blue-500',
                ]" class="font-semibold" target="_blank">code</span>,
              </h1>
              <span class="text-2xl font-semibold">
                {{ home.subtitle1 }}
                <font-awesome-icon :title="home.heart" icon="fa-solid fa-heart" :class="[
                  props.themeColor == 'light'
                    ? 'text-blue-600'
                    : 'text-blue-500',
                ]" />
              ...
              <div class="flex flex-col items-start">
                <div class="flex flex-col items-start">
                  <a href="#contact" class="flex flex-row items-center w-auto h-full min-w-auto min-h-12">
                    <font-awesome-icon icon="fa-solid fa-contact-card" :class="[
                      props.themeColor == 'light'
                        ? 'border-blue-600'
                        : 'border-blue-500',
                  ...
                  <a href="cv" class="flex flex-row items-center w-auto h-full min-w-auto min-h-12">
                    <font-awesome-icon icon="fa-solid fa-person" :class="[
                      props.themeColor == 'light'
                        ? 'border-blue-600'
                        : 'border-blue-500',
                    ]" class="fa-xl size-6 border-solid border-2 p-2 rounded-xl"></font-awesome-icon>
                ...
                <button v-for="cv of home.cv" id="dl-cv" name="dlcv" :href="cv.url"
                  ...
                  <font-awesome-icon icon="fa-solid fa-file-download" :class="[
                    props.themeColor == 'light'
                      ? 'border-blue-600'
                      : 'border-blue-500',
                  ]" class="fa-xl size-6 border-solid border-2 p-2 rounded-xl"></font-awesome-icon>
